...
export class ApiCalculation {
     ...
     */
    calcolaProductSystem = async (idProductSystem, idImpactMethod, idNewSet) => {
        try {
     ...
     */
    getStatoCalcolo = async (idCalcolo) => {
        try {
     ...
     * @returns {Json} - Json che contiene informazioni sullo stato del calcolo.
     */
    nuovoProductSystem = async (idProcess) => {
        try {
     ...
     */
    putNuovoElement = async (type, json) => {
        try {
            let url = vps + "data/" + type + "/";
            console.log(url);
            let response = await fetch(url, {
